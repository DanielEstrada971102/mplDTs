...
# the center of a cell ref to its own frame
center_cell = (0, 0, 0)

# it is possible to get the equivalent coordinates in each parent frames (Layer, SuperLayer, and Station)
...
SuperLayer_center_cell = transformer.transform(
    center_cell, from_frame="Cell", to_frame="SuperLayer"
)
...
print(
    "Cell center in Station frame: ",
    Station_center_cell,
    " -> check with config file: ",
    chamber.super_layer(1).layer(1).cell(1).local_center,
)
print(
    "Cell center in CMS frame: ",
    CMS_center_cell,
    " -> check with config file: ",
    chamber.super_layer(1).layer(1).cell(1).global_center,
)
